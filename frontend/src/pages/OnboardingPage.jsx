          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center relative z-10 bg-[#f4fcf9] px-2">
                ...
                <span className={`mt-3 text-xs font-bold transition-colors ${isActive ? 'text-[#1aa08c]' : isCompleted ? 'text-[#1aa08c]/70' : 'text-[#134e4a]/40'
                  }`}>
                  {step.label}
                </span>
              </div>
                  ...
                  <div className="flex gap-3">
                    {['Student', 'Salaried', 'Business'].map(type => (
                      <button
                        ...
                        className={`flex-1 py-3.5 rounded-xl text-sm font-bold border-2 transition-all duration-300 ${formData.employmentType === type
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${selected ? 'bg-gradient-to-br from-[#1aa08c] to-[#0d9488] text-white shadow-md' : 'bg-[#1aa08c]/10 text-[#1aa08c]'
          }`}>
          <Icon size={22} />
        </div>
        ...
        {/* Radio indicator */}
        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-1 transition-colors ${selected ? 'border-[#1aa08c]' : 'border-[#1aa08c]/30'
          }`}>
